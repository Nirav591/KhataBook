...
const sendResetPasswordMail = async (email, token) => {
  const transporter = nodemailer.createTransport({
    ...
    port: 587,
    auth: {
        user: 'rebeka.heller@ethereal.email',
        pass: 'M4YdbHF2cYcFgpSa1n'
    }
  });
  const mailOptions = {
    from: '<rebeka.heller@ethereal.email>',
    to: email,
...
exports.validateEmail = (req, res) => {
  ...
  pool.query('SELECT * FROM users WHERE email = ?', [email], async (err, user) => {
    try {
      ...
      const sent = await sendOtpMail(user[0].email, otp);

      if (sent) {       
        return res.status(200).json({ message: 'OTP is sent to your mailbox!' });      
      } else {
        return res.status(500).json({ error: 'Something went wrong while sending email.' });
      }
    } catch (err) {
...
exports.verifyOTP = (req, res) => {
  ...
  if(OTP === otp){
    ...
    return res.status(500).json({ message: 'Invalid OTP entered.Try again.' });
  } 
};
